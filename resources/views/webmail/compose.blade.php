    ...
    <div class="field">
        ...
        <div class="editor-shell">
            <div class="editor-toolbar" role="toolbar" aria-label="Email formatting">
                ...
                <button type="button" data-cmd="removeFormat">Clear format</button>
            </div>
...
@push('styles')
<style>
.compose{padding:22px}.editor-shell{border:1px solid var(--line);border-radius:15px;overflow:hidden;background:#041a23}.editor-toolbar{display:flex;align-items:center;gap:5px;flex-wrap:wrap;padding:9px;border-bottom:1px solid var(--line);background:#071e28}.editor-toolbar button{border:1px solid transparent;background:transparent;color:#b9d0d7;border-radius:8px;padding:7px 9px;font-size:12px;cursor:pointer}.editor-toolbar button:hover,.editor-toolbar button.active{background:#0b3542;border-color:#1b5968;color:#fff}.toolbar-sep{width:1px;height:24px;background:var(--line);margin:0 3px}.visual-editor{min-height:360px;padding:18px;color:#e9f6fa;outline:none;line-height:1.7;overflow-wrap:anywhere}.visual-editor:focus{box-shadow:inset 0 0 0 1px rgba(34,195,223,.35)}.visual-editor:empty:before{content:'Write your message...';color:#65828e}.visual-editor img{max-width:100%;height:auto;border-radius:8px}.visual-editor a{color:#22c3df}.visual-editor blockquote{border-left:3px solid #287384;padding-left:14px;margin-left:0;color:#9db8c0}.editor-hint{color:var(--muted);font-size:12px;margin:8px 0 0;line-height:1.5}.compose-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:18px}.compose-actions .btn{text-decoration:none}.field input{min-height:46px}
@media(max-width:650px){.compose{padding:15px}.visual-editor{min-height:300px;padding:14px}.editor-toolbar{gap:3px}.editor-toolbar button{padding:7px 8px}.toolbar-sep{display:none}.compose-actions{justify-content:stretch}.compose-actions .btn{flex:1}}
</style>
@endpush
...
(function(){
    ...
    const initialBody=@json(old('body',$initialBody));
    editor.innerHTML=initialBody||'';

    function focusEditor(){editor.focus();}
    document.querySelectorAll('[data-cmd]').forEach(button=>button.addEventListener('click',()=>{focusEditor();document.execCommand(button.dataset.cmd,false,null);editor.dispatchEvent(new Event('input'));}));
    ...
    form.addEventListener('submit',function(){body.value=editor.innerHTML.trim();if(!body.value||body.value==='<br>'){event.preventDefault();editor.focus();}});
})();
</script>
@endpush
@endsection
