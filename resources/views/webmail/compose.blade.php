...
@push('styles')<style>.compose{padding:22px;min-width:0;max-width:100%;overflow:hidden}.recipient-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.recipient-grid .field:first-child{grid-column:1/-1}.field{margin-bottom:14px}.field label{display:block;margin-bottom:7px;color:#a9c2ca;font-weight:700;font-size:14px}.field input{width:100%;min-height:46px;border:1px solid var(--line);background:#041a23;color:var(--text);border-radius:13px;padding:13px 14px;outline:none}.field input:focus{border-color:var(--accent)}.compose-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:18px}.compose-actions .btn{text-decoration:none}@media(max-width:900px){.recipient-grid{grid-template-columns:1fr}.recipient-grid .field:first-child{grid-column:auto}}
@media(max-width:760px){.compose{padding:14px;border-radius:16px}.recipient-grid{gap:10px}.compose-actions{justify-content:stretch;flex-direction:column-reverse}.compose-actions .btn{width:100%}.hero{flex-direction:column;align-items:stretch}.hero>.btn{width:100%}}</style>@endpush
@endsection