...
$navIcon=trim((string) $item->icon) !== '' ? trim((string) $item->icon) : ($hasChildren ? 'fa-folder-tree' : 'fa-circle-dot');
@endphp
@if($hasChildren)<div class="nav-group {{ $navOpen?'open':'' }}"><button type="button" class="nav-parent" aria-expanded="{{ $navOpen?'true':'false' }}"><span class="nav-icon"><i class="fa-solid {{ $navIcon }}"></i></span><span>{{ $item->displayLabel() }}</span><i class="fa-solid fa-chevron-down nav-chevron"></i></button><div class="nav-sub">@foreach($item->children as $child)@include('layouts._dashboard-navigation-item',['item'=>$child])@endforeach</div></div>
@else<a class="{{ $isActive?'active':'' }}" href="{{ $item->url }}" @if($item->target==='_blank') target="_blank" rel="noopener noreferrer" @endif><span class="nav-icon"><i class="fa-solid {{ $navIcon }}"></i></span><span>{{ $item->displayLabel() }}</span></a>@endif
