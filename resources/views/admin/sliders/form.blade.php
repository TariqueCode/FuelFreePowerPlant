...
<div class="grid">
<div class="full"><label>Slider image {{ $slider->exists?'(leave empty to keep current)':'' }}</label><input type="file" name="image" accept="image/jpeg,image/png,image/webp,image/avif" {{ $slider->
<div class="upload-guide" role="note"><i class="fa-solid fa-ruler-combined"></i><div><strong>Recommended slider image</strong><span>Use <b>1600 × 700 px</b> (2.29:1) for the best desktop, tablet and mobile crop. JPG or WebP is recommended.</span></div></div>exists?'':'required' }}></div>
...
<style>.upload-guide{display:flex;align-items:flex-start;gap:11px;margin:8px 0 14px;padding:11px 13px;border:1px solid rgba(72,216,241,.18);border-radius:12px;background:rgba(72,216,241,.045);color:#8aa7af;font-size:10px;line-height:1.5}.upload-guide i{color:#58cfe7;margin-top:2px}.upload-guide div{display:flex;flex-direction:column;gap:2px}.upload-guide strong{color:#dff8fc;font-size:11px}.upload-guide b{color:#7ee4f4;font-weight:800}</style>