    ...
    <div class="file-list" id="file-list">
        @forelse($documents as $document)
            <article class="file-row">
                ...
                <div class="file-actions"><a href="{{ route('admin.documents.download', $document) }}">Download</a><details class="menu"><summary>⋮</summary><div class="menu-pop right"><button type="button" onclick="openDialog('rename-file-{{ $document->id }}')">Rename</button><button type="button" onclick="openDialog('move-file-{{ $document->id }}')">Move</button><button type="button" onclick="openDialog('copy-file-{{ $document->id }}')">Copy</button><button class="danger" type="button" onclick="openDialog('delete-file-{{ $document->id }}')">Delete</button></div></details></div>
            </article>
...
<dialog id="upload-modal"><form method="POST" action="{{ route('admin.documents.store') }}" enctype="multipart/form-data">@csrf<h2>Upload files</h2><p>Select any file supported by your server.</p><input type="file" name="file" required>@if($folder)<input type="hidden" name="folder_id" value="{{ $folder->id }}">@endif<div class="modal-note">No application-level file-type or size cap. PHP/web-server limits and available storage remain the hard boundary.</div><div class="modal-actions"><button type="button" onclick="this.closest('dialog').close()">Cancel</button><button class="primary" type="submit">Upload securely</button></div></form></dialog>
...
@media(max-width:440px){.folder-grid{grid-template-columns:1fr}.storage-top{align-items:flex-start;flex-direction:column}.storage-right{text-align:left}.manager-actions{grid-template-columns:1fr 1fr}.view-btn{display:none}.file-actions{justify-content:flex-start}.file-actions>a{font-size:10px}}
</style>
@endpush
