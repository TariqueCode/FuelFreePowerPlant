...
<div class="social-grid">
    <section class="panel form-panel">
        ...
        <form method="POST" action="{{ route('admin.social-links.store') }}" class="social-form">
            ...
            <div class="form-row">
                <label>Order
                    <input type="number" name="sort_order" min="0" max="9999" value="{{ old('sort_order', $links->count() + 1) }}">
                </label>
            ...
            <article class="social-item" draggable="true" data-id="{{ $link->id }}">
                ...
                <form method="POST" action="{{ route('admin.social-links.update', $link) }}" class="item-form">
                    ...
                    <div class="item-fields">
                        ...
                        <label>Order
                            <input type="number" name="sort_order" min="0" max="9999" value="{{ $link->sort_order }}">
                        </label>
                    </div>
...
@push('styles')
<style>
.social-hero{display:flex;align-items:flex-end;justify-content:space-between;gap:24px;padding:6px 0 22px}.social-hero h1{font-size:clamp(30px,4vw,46px);margin:7px 0}.social-hero p{max-width:720px;color:#7f9eaa;font-size:12px;line-height:1.7}.social-count{min-width:110px;padding:14px 16px;border:1px solid var(--line);border-radius:15px;background:rgba(67,194,229,.05);text-align:center}.social-count strong{display:block;font-size:25px}.social-count span{color:#7595a2;font-size:9px}.flash{padding:11px 13px;margin-bottom:12px;border-radius:11px;background:rgba(67,194,137,.1);color:#a8e5ca;font-size:10px}.flash.error{background:rgba(255,90,110,.08);color:#ffb1ba}.social-grid{display:grid;grid-template-columns:360px 1fr;gap:13px}.panel{border:1px solid var(--line);border-radius:20px;padding:21px;background:linear-gradient(145deg,rgba(9,39,55,.86),rgba(5,22,32,.92));box-shadow:0 16px 38px rgba(0,0,0,.13)}.panel-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.panel-head h2{margin:5px 0;font-size:17px}.panel-head>i{color:#5fd5ef}.data-badge{font-size:9px;color:#86a8b5;padding:6px 8px;border:1px solid var(--line);border-radius:999px}.social-form{display:grid;gap:12px;margin-top:18px}.social-form label,.item-fields label{display:grid;gap:6px;color:#87a5b0;font-size:10px}.social-form input,.social-form select,.item-fields input,.item-fields select{width:100%;border:1px solid rgba(104,204,235,.14);background:#061923;color:#e5f5f8;border-radius:10px;padding:10px 11px;font-size:11px;outline:none}.social-form select,.item-fields select{appearance:auto}.social-form input:focus,.social-form select:focus,.item-fields input:focus,.item-fields select:focus{border-color:rgba(91,215,239,.45);box-shadow:0 0 0 3px rgba(91,215,239,.07)}.form-row{display:grid;grid-template-columns:1fr 1.4fr;gap:10px}.switch-row{display:flex!important;align-items:center;justify-content:space-between;padding:0 8px}.switch-row input,.mini-switch input{display:none}.switch-row i{width:36px;height:20px;border-radius:99px;background:#172a34;position:relative;border:1px solid var(--line)}.switch-row i:after{content:"";position:absolute;width:14px;height:14px;top:2px;left:2px;border-radius:50%;background:#6f8b95;transition:.2s}.switch-row input:checked+i{background:#1b9fbd}.switch-row input:checked+i:after{left:18px;background:#eaffff}.primary-btn,.save-btn,.delete-btn{border:1px solid rgba(104,204,235,.16);border-radius:10px;cursor:pointer}.primary-btn{min-height:40px;background:linear-gradient(100deg,#1e9fc0,#2aafca);color:#fff;font-weight:750}.platform-preview{display:flex;align-items:center;gap:11px;padding:10px 12px;border:1px solid rgba(104,204,235,.10);border-radius:12px;background:rgba(67,194,229,.035)}.platform-preview .preview-icon{width:34px;height:34px;border-radius:10px;display:grid;place-items:center;background:rgba(67,194,229,.08);color:var(--brand-color,#51d8f0);font-size:16px}.platform-preview strong,.platform-preview small{display:block}.platform-preview strong{font-size:11px;color:#dff4f7}.platform-preview small{font-size:8px;color:#668692;margin-top:2px}.platform-note{display:flex;gap:8px;margin-top:15px;padding:10px 11px;border-radius:11px;background:rgba(67,194,229,.035);border:1px solid rgba(104,204,235,.08);color:#708e99;font-size:9px;line-height:1.55}.platform-note i{color:#59cee8;margin-top:1px}.drag-hint{display:flex;align-items:center;gap:7px;margin:13px 0 2px;color:#678792;font-size:9px}.drag-hint i{color:#5bb8ca}.social-list{display:grid;gap:8px}.social-item{display:grid;grid-template-columns:24px 46px minmax(0,1fr) 38px;gap:11px;align-items:start;padding:13px 0;border-top:1px solid rgba(255,255,255,.055);transition:transform .18s,border-color .18s,opacity .18s}.social-item:hover{background:rgba(67,194,229,.018)}.social-item.dragging{opacity:.45;transform:scale(.99)}.drag-handle{display:grid;place-items:center;height:42px;color:#537883;cursor:grab}.drag-handle:active{cursor:grabbing}.social-preview{width:46px;height:46px;display:grid;place-items:center;border-radius:13px;background:rgba(67,194,229,.07);border:1px solid rgba(104,204,235,.13);color:#76d9ef;font-size:18px;transition:color .2s,background .2s,border-color .2s}.social-item:hover .social-preview{color:var(--brand-color);background:color-mix(in srgb,var(--brand-color) 10%,transparent);border-color:color-mix(in srgb,var(--brand-color) 28%,transparent)}.item-form{min-width:0}.item-fields{display:grid;grid-template-columns:1.05fr 2fr 70px;gap:8px}.item-actions{display:flex;align-items:center;gap:8px;margin-top:9px}.mini-switch{display:flex;align-items:center;gap:7px;color:#7696a2;font-size:9px;cursor:pointer}.mini-switch span:before{content:"";display:inline-block;width:7px;height:7px;border-radius:50%;margin-right:5px;background:#526974}.mini-switch input:checked+span:before{background:#57d8aa}.save-btn{padding:8px 11px;background:rgba(67,194,229,.07);color:#a9dbe4;font-size:10px}.delete-btn{width:38px;height:38px;background:rgba(255,80,100,.04);color:#d98a96}.empty{min-height:220px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;color:#6e8f9b;text-align:center}.empty i{font-size:28px;color:#58cde9}.empty strong{color:#9bb7c1;font-size:13px}.empty span{font-size:10px}
@media(max-width:1000px){.social-grid{grid-template-columns:1fr}}
@media(max-width:650px){.social-hero{align-items:flex-start;flex-direction:column}.social-count{align-self:flex-end}.social-grid{gap:10px}.panel{padding:16px}.item-fields{grid-template-columns:1fr 1fr}.item-fields label:nth-child(2){grid-column:1/-1}.social-item{grid-template-columns:22px 42px minmax(0,1fr) 34px;gap:8px}.social-preview{width:42px;height:42px}.delete-btn{width:34px;height:34px}.form-row{grid-template-columns:1fr}.item-actions{justify-content:space-between}}
</style>
@endpush
