...
<article class="content-card {{ $type==='company' ? 'company-card' : '' }} {{ $type==='news' ? 'news-card' : '' }}" data-edit-url="{{ route('admin.site-content.edit',$item) }}">
    ...
    <div class="info">
        ...
        @if($type==='news' && $item->excerpt)<div class="excerpt">{{ \Illuminate\Support\Str::limit($item->excerpt,150) }}</div>@endif
...
@empty<div class="empty"><i class="fa-regular fa-newspaper"></i><strong>No {{ $type==='news' ? 'news or notices' : 'content' }} yet</strong><span>Use the button above to create the first entry.</span></div>@endforelse
</div>
