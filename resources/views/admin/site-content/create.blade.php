...
<div class="full resource-builder-suite">
    <div class="resource-suite-head">
        <div><span class="suite-kicker">UNIFIED PAGE BUILDER</span><strong>Advanced Resource Builder</strong><small>Everything from Content Pages is now available here without removing the existing Resource editor or PDF workflow.</small></div>
        <div class="resource-suite-actions">
            <button type="button" class="suite-tab active" data-suite-tab="builder">Page Builder</button>
            <button type="button" class="suite-tab" data-suite-tab="seo">SEO &amp; Framework</button>
        </div>
    </div>
    <div class="suite-panel active" data-suite-panel="builder">
        <div class="builder-toolbar"><strong>Visual Sections</strong><button type="button" id="resource-add-block" class="builder-add"><i class="fa-solid fa-plus"></i> Add section</button></div>
        <div id="resource-blocks-list"></div>
        <input type="hidden" name="builder_blocks" id="resource-builder-json" value='{{ old("builder_blocks", $item->builder_blocks ? json_encode($item->builder_blocks) : "[]") }}'>
        <small class="hint">Build structured sections above the existing advanced rich-text content. The original editor below remains fully intact.</small>
    </div>
    <div class="suite-panel" data-suite-panel="seo">
        <div class="seo-grid">
            <div><label>SEO Title</label><input name="meta_title" value="{{ old('meta_title',$item->meta_title) }}" maxlength="255" placeholder="Search engine title"></div>
            <div><label>Template</label><select name="template"><option value="default" @selected(old('template',$item->template ?: 'default')==='default')>Default Resource</option><option value="article" @selected(old('template',$item->template)==='article')>Article</option><option value="landing" @selected(old('template',$item->template)==='landing')>Landing Page</option></select></div>
            <div class="full"><label>SEO Description</label><textarea name="meta_description" rows="3" maxlength="1000" placeholder="Professional search description">{{ old('meta_description',$item->meta_description) }}</textarea></div>
        </div>
        <div class="framework-options">
            <div class="framework-title">Global Framework</div>
            <small>Control whether this Resource uses the site's shared shell.</small>
            <label class="check-label"><input type="checkbox" name="use_global_framework" value="1" @checked(old('use_global_framework',$item->exists ? $item->use_global_framework : true))> <span>Use Global Framework</span></label>
            <div class="framework-sub">
                <label class="check-label"><input type="checkbox" name="use_global_header" value="1" @checked(old('use_global_header',$item->exists ? $item->use_global_header : true))> <span>Use Global Header</span></label>
                <label class="check-label"><input type="checkbox" name="use_global_footer" value="1" @checked(old('use_global_footer',$item->exists ? $item->use_global_footer : true))> <span>Use Global Footer</span></label>
            </div>
        </div>
    </div>
</div>
...
@if($item->exists)<button type="button" class="attachment-upload" id="attachment-upload"><i class="fa-solid fa-file-arrow-up"></i> {{ $item->attachment_path ? 'Replace PDF' : 'Upload PDF' }}</button>@else<div class="attachment-save-note"><i class="fa-solid fa-circle-info"></i> Save this resource first to attach a PDF.</div>@endif</div>
...
<script>
(()=> {
 const list=document.getElementById('resource-blocks-list'), hidden=document.getElementById('resource-builder-json'), add=document.getElementById('resource-add-block');
 if(!list||!hidden||!add) return;
 let blocks=[]; try{const x=JSON.parse(hidden.value||'[]');blocks=Array.isArray(x)?x:[]}catch(e){}
 ...
 const sync=()=>{hidden.value=JSON.stringify(blocks);render()};
 function render(){
   list.innerHTML='';
   blocks.forEach((b,i)=>{
     const row=document.createElement('div'); row.className='resource-block-row'; row.draggable=true; row.dataset.i=i;
     row.innerHTML='<div class="resource-block-head"><span class="grip">☷</span><select class="rb-type">'+Object.entries(types).map(([k,v])=>'<option value="'+k+'" '+(b.type===k?'selected':'')+'>'+v+'</option>').join('')+'</select><button type="button" class="rb-remove">Remove</button></div><div class="resource-block-fields"><input class="rb-title" placeholder="Section title" value="'+safe(b.title)+'"><textarea class="rb-content" rows="3" placeholder="Section content (HTML supported)">'+safe(b.content)+'</textarea><input class="rb-image" placeholder="Image URL (optional)" value="'+safe(b.image)+'"><input class="rb-url" placeholder="Link / Button URL (optional)" value="'+safe(b.url)+'"><label class="check-label"><input class="rb-visible" type="checkbox" '+(b.visible!==false?'checked':'')+'> <span>Visible</span></label></div>';
     row.querySelector('.rb-type').onchange=e=>{b.type=e.target.value;sync()};
     row.querySelector('.rb-title').oninput=e=>{b.title=e.target.value;hidden.value=JSON.stringify(blocks)};
     row.querySelector('.rb-content').oninput=e=>{b.content=e.target.value;hidden.value=JSON.stringify(blocks)};
     row.querySelector('.rb-image').oninput=e=>{b.image=e.target.value;hidden.value=JSON.stringify(blocks)};
     row.querySelector('.rb-url').oninput=e=>{b.url=e.target.value;hidden.value=JSON.stringify(blocks)};
     row.querySelector('.rb-visible').onchange=e=>{b.visible=e.target.checked;hidden.value=JSON.stringify(blocks)};
     row.querySelector('.rb-remove').onclick=()=>{blocks.splice(i,1);sync()};
     row.ondragstart=()=>row.classList.add('dragging'); row.ondragover=e=>e.preventDefault(); row.ondrop=()=>{const from=+row.dataset.i;if(from===i)return;const x=blocks.splice(from,1)[0];blocks.splice(i,0,x);sync()}; row.ondragend=()=>row.classList.remove('dragging');
     list.appendChild(row);
   });
 }
 add.onclick=()=>{blocks.push({type:'text',title:'',content:'',image:'',url:'',visible:true});sync()};
 document.querySelectorAll('[data-suite-tab]').forEach(tab=>tab.addEventListener('click',()=>{document.querySelectorAll('[data-suite-tab]').forEach(x=>x.classList.toggle('active',x===tab));document.querySelectorAll('[data-suite-panel]').forEach(x=>x.classList.toggle('active',x.dataset.suitePanel===tab.dataset.suiteTab));}));
 render();
})();
</script>
@endpush
...
@push('styles')
<style>
.resource-builder-suite{border:1px solid rgba(67,194,229,.18);border-radius:16px;background:linear-gradient(145deg,rgba(67,194,229,.055),rgba(255,255,255,.018));overflow:hidden}
.resource-suite-head{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:15px 16px;border-bottom:1px solid rgba(67,194,229,.12)}
.resource-suite-head strong{display:block;color:#e7f8fb;font-size:13px;margin:4px 0}.suite-kicker{font-size:8px;letter-spacing:.15em;color:#58d3ec}.resource-suite-head small{display:block;color:#718f9a;font-size:9px;line-height:1.5;max-width:680px}.resource-suite-actions{display:flex;gap:5px;flex:0 0 auto}.suite-tab{border:1px solid var(--line);background:#061923;color:#7f9da7;border-radius:9px;padding:9px 11px;font-size:9px;font-weight:800;cursor:pointer}.suite-tab.active{color:#eafaff;border-color:rgba(67,194,229,.35);background:rgba(67,194,229,.1)}
.suite-panel{display:none;padding:15px}.suite-panel.active{display:block}.builder-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px;color:#dff5f8;font-size:11px}.builder-add{border:1px solid rgba(67,194,229,.28);border-radius:9px;background:rgba(67,194,229,.08);color:#cceff6;padding:8px 11px;font-size:9px;font-weight:800;cursor:pointer}.resource-block-row{border:1px solid rgba(67,194,229,.12);border-radius:12px;background:#061923;margin-bottom:9px;overflow:hidden}.resource-block-row.dragging{opacity:.5}.resource-block-head{display:flex;align-items:center;gap:7px;padding:8px;border-bottom:1px solid rgba(67,194,229,.1)}.resource-block-head .grip{color:#5e8490;cursor:grab}.resource-block-head select{flex:1;height:34px}.rb-remove{border:1px solid rgba(255,99,113,.15);border-radius:8px;background:rgba(255,99,113,.04);color:#ff9eaa;padding:7px 9px;font-size:9px;cursor:pointer}.resource-block-fields{display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:10px}.resource-block-fields .rb-content{grid-column:1/-1}.resource-block-fields .rb-visible{width:16px;height:16px}.framework-options{margin-top:12px}.framework-sub{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px;padding-left:24px}@media(max-width:650px){.resource-suite-head{align-items:stretch;flex-direction:column}.resource-suite-actions{width:100%}.suite-tab{flex:1}.resource-block-fields{grid-template-columns:1fr}.resource-block-fields .rb-content{grid-column:auto}.framework-sub{grid-template-columns:1fr;padding-left:0}}
</style>
@endpush
@endif
@push('styles')<style>
