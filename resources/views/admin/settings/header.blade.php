...
<x-admin.card>
    ...
    <div class="header-builder-grid">
      ...
      <x-admin.setting-field label="Logo corner radius (px)" id="header-logo-radius"><input class="admin-input" type="number" name="header.logo_radius" min="0" max="24" value="{{ old('header.logo_radius',$settings['header.logo_radius']??'8') }}" required></x-admin.setting-field>
      <x-admin.setting-field label="Logo fit" id="header-logo-fit"><select class="admin-input" name="header.logo_fit"><option value="contain" @selected(old('header.logo_fit',$settings['header.logo_fit']??'contain')==='contain')>Contain</option><option value="cover" @selected(old('header.logo_fit',$settings['header.logo_fit']??'contain')==='cover')>Cover</option><option value="fill" @selected(old('header.logo_fit',$settings['header.logo_fit']??'contain')==='fill')>Fill</option></select></x-admin.setting-field>
      <label class="admin-toggle"><input type="hidden" name="header.brand_name_visible" value="0"><input type="checkbox" name="header.brand_name_visible" value="1" @checked(old('header.brand_name_visible',$settings['header.brand_name_visible']??'1')==='1')><span>Show brand name</span></label>
      ...
      <x-admin.setting-field label="Social style" id="header-social-style"><select class="admin-input" name="header.social_style"><option value="soft" @selected(old('header.social_style',$settings['header.social_style']??'soft')==='soft')>Soft</option><option value="outline" @selected(old('header.social_style',$settings['header.social_style']??'soft')==='outline')>Outline</option><option value="plain" @selected(old('header.social_style',$settings['header.social_style']??'soft')==='plain')>Plain</option></select></x-admin.setting-field>
      <x-admin.setting-field label="Social alignment" id="header-social-alignment"><select class="admin-input" name="header.social_alignment"><option value="left" @selected(old('header.social_alignment',$settings['header.social_alignment']??'right')==='left')>Left</option><option value="center" @selected(old('header.social_alignment',$settings['header.social_alignment']??'right')==='center')>Center</option><option value="right" @selected(old('header.social_alignment',$settings['header.social_alignment']??'right')==='right')>Right</option></select></x-admin.setting-field>
...
<div class="header-mobile-section-title">Mobile header</div>
    <div class="header-mobile-grid">
      ...
      <x-admin.setting-field label="Mobile menu button (px)" id="header-mobile-menu-size"><input class="admin-input" type="number" name="header.mobile_menu_size" min="34" max="48" value="{{ old('header.mobile_menu_size',$settings['header.mobile_menu_size']??'38') }}" required></x-admin.setting-field>
      <x-admin.setting-field label="Mobile menu radius (px)" id="header-mobile-nav-radius"><input class="admin-input" type="number" name="header.mobile_nav_radius" min="0" max="24" value="{{ old('header.mobile_nav_radius',$settings['header.mobile_nav_radius']??'14') }}" required></x-admin.setting-field>
      <x-admin.setting-field label="Mobile menu padding (px)" id="header-mobile-nav-padding"><input class="admin-input" type="number" name="header.mobile_nav_padding" min="0" max="20" value="{{ old('header.mobile_nav_padding',$settings['header.mobile_nav_padding']??'8') }}" required></x-admin.setting-field>
    </div>
