...
@csrf
<section class="settings-card"><header class="card-heading"><div><span class="eyebrow">IDENTITY</span><h2>Company identity</h2><p>Global brand information shared across the public website and platform.</p></div><span class="heading-icon"><i class="fa-solid fa-building"></i></span></header>
<div class="identity-card"><div class="logo-preview">@if($settings['company.logo_path'])<img src="{{ asset('storage/'.$settings['company.logo_path']) }}" alt="Company logo">@else<i class="fa-solid fa-building"></i>@endif</div><div class="identity-copy"><strong>{{ $settings['company.name'] }}</strong><span>Use a clear square or transparent logo for the best result.</span><label class="upload-button"><i class="fa-solid fa-cloud-arrow-up"></i><span>Change logo</span><input type="file" name="company[logo]" accept="image/png,image/jpeg,image/webp,image/svg+xml"></label></div></div>
<div class="field-grid"><label><span>Company name</span><input name="company[name]" value="{{ old('company.name',$settings['company.name']) }}" required maxlength="150"></label><label><span>Company domain</span><input name="company[domain]" value="{{ old('company.domain',$settings['company.domain']) }}" required maxlength="255"></label><label class="full"><span>Tagline</span><input name="company[tagline]" value="{{ old('company.tagline',$settings['company.tagline']) }}" maxlength="255"></label><label><span>Timezone</span><input name="company[timezone]" value="{{ old('company.timezone',$settings['company.timezone']) }}" required placeholder="Asia/Dhaka"></label></div></section>
