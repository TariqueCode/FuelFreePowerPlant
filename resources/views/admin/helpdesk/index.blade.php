...
<section class="hero">
    ...
    <h1>Help Desk</h1>
    <p>All website contact and career messages are handled here. Messages stay inside the application database; replies are sent from the configured official mailbox.</p>
</section>
...
<div class="grid">
    ...
    <article class="card"><span class="card-label">Contact</span><strong class="card-value">{{ number_format($contactCount) }}</strong><span class="card-note">Website inquiries</span></article>
    <article class="card"><span class="card-label">Career</span><strong class="card-value">{{ number_format($careerCount) }}</strong><span class="card-note">Applications</span></article>
</div>
    ...
    <div class="table-wrap">
        <table>
            ...
            @forelse($items as $item)
                <tr>
                    <td><span class="badge">{{ ucfirst($item->type) }}</span></td>
                    ...
                    <td><a class="action" href="{{ $item->route }}">Open</a></td>
                </tr>
            @empty
