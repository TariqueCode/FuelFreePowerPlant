...
<div class="hd-shell">
    ...
    <section class="hd-toolbar">
        ...
        <div class="hd-filters">
            <a class="{{ $channel==='all'?'active':'' }}" href="{{ route('admin.helpdesk',array_filter(['q'=>$search,'status'=>$status!=='all'?$status:null])) }}">All <em>{{ number_format($contactCount+$careerCount+$emailCount) }}</em></a>
