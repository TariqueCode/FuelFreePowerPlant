    ...
    @foreach($sections as $index => $section)
        ...
        <article class="section-card" draggable="true" data-key="{{ $section->key }}" data-state="{{ $section->is_enabled ? 'visible' : 'hidden' }}">
            <div class="section-row">
                <div class="order-column" aria-label="Drag to reorder {{ $titles[$section->key] ?? $section->label }}">
                    <span class="order-number">{{ sprintf('%02d',$index + 1) }}</span>
                    <span class="drag-handle" title="Drag to reorder" aria-label="Drag to reorder"><i class="fa-solid fa-grip-vertical"></i></span>
                </div>
