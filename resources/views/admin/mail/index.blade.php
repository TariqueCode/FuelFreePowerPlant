...
<section class="website-inbox">
    <div class="website-inbox-head">
        <div><span class="eyebrow">WEBSITE MESSAGES</span><h2>Contact &amp; Career</h2><p>These submissions live in the website database. Replies are sent through the configured official mailbox and are not copied into the cPanel mailbox.</p></div>
    </div>
    <div class="website-grid">
        <div class="website-panel">
            <div class="website-panel-head"><strong>Contact</strong><span>{{ $inquiries->count() }}</span></div>
            @forelse($inquiries as $inquiry)
                <a href="{{ route('admin.inquiries.show',$inquiry) }}" class="website-row">
                    <span class="website-dot {{ $inquiry->status==='new'?'new':'' }}"></span>
                    <span><strong>{{ $inquiry->name }}</strong><small>{{ $inquiry->subject }}</small></span>
                    <time>{{ $inquiry->created_at->format('d M') }}</time>
                </a>
            @empty
                <div class="website-empty">No Contact messages yet.</div>
            @endforelse
        </div>
        <div class="website-panel">
            <div class="website-panel-head"><strong>Career</strong><span>{{ $applications->count() }}</span></div>
            @forelse($applications as $application)
                <a href="{{ route('admin.career-applications.show',$application) }}" class="website-row">
                    <span class="website-dot {{ $application->status==='new'?'new':'' }}"></span>
                    <span><strong>{{ $application->name }}</strong><small>{{ $application->position ?: 'General application' }}</small></span>
                    <time>{{ $application->created_at->format('d M') }}</time>
                </a>
            @empty
                <div class="website-empty">No Career applications yet.</div>
            @endforelse
        </div>
    </div>
</section>
...
.website-inbox{margin-top:14px;border:1px solid var(--line);border-radius:18px;background:rgba(5,22,32,.78);padding:14px}.website-inbox-head h2{font-size:18px;margin:5px 0}.website-inbox-head p{margin:0;color:#7898a5;font-size:9px;line-height:1.6;max-width:760px}.website-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px}.website-panel{border:1px solid rgba(86,210,238,.1);border-radius:13px;overflow:hidden;background:rgba(67,194,229,.018)}.website-panel-head{display:flex;justify-content:space-between;padding:10px 12px;border-bottom:1px solid rgba(255,255,255,.05);color:#cbe8ed;font-size:10px}.website-panel-head span{color:#6fd8ef}.website-row{display:grid;grid-template-columns:7px minmax(0,1fr) auto;gap:9px;align-items:center;padding:10px 12px;border-bottom:1px solid rgba(255,255,255,.04);text-decoration:none;color:#d8eef2}.website-row:last-child{border-bottom:0}.website-row:hover{background:rgba(67,194,229,.05)}.website-row strong,.website-row small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.website-row strong{font-size:9px}.website-row small{font-size:8px;color:#6e8d98;margin-top:3px}.website-row time{font-size:7px;color:#5f7d88}.website-dot{width:7px;height:7px;border-radius:50%;background:#4e6872}.website-dot.new{background:#55d7ef;box-shadow:0 0 8px rgba(85,215,239,.5)}.website-empty{padding:22px;text-align:center;color:#7898a5;font-size:9px}@media(max-width:700px){.website-grid{grid-template-columns:1fr}}
</style>
@endpush
