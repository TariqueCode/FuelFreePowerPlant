    ...
    ->withRouting(
        ...
        then: function (): void {
            Route::middleware(['web', 'auth', 'permission:website.view'])->prefix('admin/profile-builder')->name('admin.profile-builder.')->group(function (): void {
                ...
                Route::get('/folders/{folder}', fn (\App\Models\ManagementProfileFolder $folder) => view('admin.management.folder-show', ['folder' => $folder->loadCount('profiles')->load(['profiles' => fn ($query) => $query->orderBy('sort_order')->orderBy('id')])]))->whereNumber('folder')->name('folders.show');
