...
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach ($roles as $role) DB::table('roles')->updateOrInsert(['slug'=>$role['slug']], $role);
        ...
        foreach ($permissions as $permission) DB::table('permissions')->updateOrInsert(['slug'=>$permission['slug']], $permission);
        ...
        $mailPermissions = DB::table('permissions')->whereIn('slug',['mail.view','mail.manage'])->pluck('id');
        $careerPermissions = DB::table('permissions')->whereIn('slug',['career.view','career.manage'])->pluck('id');
        $allNew = $mailPermissions->merge($careerPermissions)->unique();

        foreach ($mailPermissions as $permissionId) DB::table('permission_role')->updateOrInsert(['permission_id'=>$permissionId,'role_id'=>$mailRole],[]);
        foreach ($careerPermissions as $permissionId) DB::table('permission_role')->updateOrInsert(['permission_id'=>$permissionId,'role_id'=>$careerRole],[]);
        foreach ($allNew as $permissionId) DB::table('permission_role')->updateOrInsert(['permission_id'=>$permissionId,'role_id'=>$superRole],[]);
    }

    public function down(): void
    {
        $roleIds = DB::table('roles')->whereIn('slug',['mail-manager','career-manager'])->pluck('id');
        ...
        $permissionIds = DB::table('permissions')->whereIn('slug',['mail.view','mail.manage','career.view','career.manage'])->pluck('id');
