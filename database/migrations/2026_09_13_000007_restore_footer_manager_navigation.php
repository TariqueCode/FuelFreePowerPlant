...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $profile = NavigationMenuItem::query()
            ->where('menu', 'dashboard')
            ->where('area', 'dashboard')
            ->where('source_key', 'route:admin.management.index')
            ->first();
        if ($profile) {
            $profile->update([
                'label' => 'Profile Builder',
                'url' => '/admin/profile-builder',
                'route_name' => 'admin.profile-builder.index',
                'source_key' => 'route:admin.profile-builder.index',
                'source_type' => 'route',
        ...
        $footer = NavigationMenuItem::query()
            ...
            ->where(function ($query): void {
                $query->where('label', 'Footer Manager')
                    ->orWhere('url', '/admin/settings?section=footer');
            })
            ->first();
        ...
        $values = [
            ...
            'label_override' => null,
            'url' => '/admin/settings?section=footer',
            'route_name' => null,
            'target' => '_self',
        ...
        if ($footer) $footer->update($values);
        else NavigationMenuItem::create($values);
