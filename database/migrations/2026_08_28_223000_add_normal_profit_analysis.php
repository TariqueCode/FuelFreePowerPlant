...
<tr><td>Annual net operating cash flow</td><td>BDT 4.38 Crore</td></tr>
</tbody>
</table>
...
HTML;
        ...
        if (str_contains($item->content, $placeholder)) {
            $content = str_replace($placeholder, $normal, $item->content);
    ...
    public function down(): void
    {
        ...
        $normalStart = strpos($item->content, '<h2 id="normal">Normal</h2>');
        $conservativeStart = strpos($item->content, '<h2 id="conservative">Conservative</h2>');

        if ($normalStart !== false && $conservativeStart !== false && $normalStart < $conservativeStart) {
            $replacement = '<h2 id="normal">Normal</h2>\n<p>Normal scenario content will be added in the next verified step.</p>\n\n';
            $content = substr($item->content, 0, $normalStart)
                . $replacement
                . substr($item->content, $conservativeStart);
