...
class CpanelEmailService
{
    ...
    {
        if (! $this->configured()) {
            throw new RuntimeException('cPanel mail provisioning is not configured.');
        }

        [$local, $domain] = $this->splitAddress($address);
        if (strcasecmp($domain, (string) config('cpanel.domain')) !== 0) {
            throw new RuntimeException('The mailbox domain is not configured for this cPanel account.');
        }

        $result = $this->request('add_pop', [
            'email' => $local,
            'domain' => $domain,
            'password' => $password,
        ...
        return $this->message($result, 'Mailbox created in cPanel.');
    }

    public function delete(EmailAccount $account): string
    {
        if (! $this->configured()) {
            return 'Mailbox record removed. cPanel provisioning is not configured.';
        }

        $result = $this->request('delete_pop', ['email' => $account->address]);
        return $this->message($result, 'Mailbox removed from cPanel.');
    }
    ...
    private function request(string $function, array $query): array
    {
        ...
        if (! $response->successful()) {
            throw new RuntimeException('cPanel API returned HTTP ' . $response->status() . '.');
        }

        $payload = $response->json();
        if (! is_array($payload)) {
            throw new RuntimeException('Invalid response received from cPanel.');
        }
