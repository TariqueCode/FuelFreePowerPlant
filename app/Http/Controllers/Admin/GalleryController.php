...
class GalleryController extends Controller
{
    ...
    public function uploadMedia(Request $request): JsonResponse{$data=$request->validate(['media'=>['required','file','mimes:jpg,jpeg,png,webp,gif,mp4,webm,mov','max:'.$this->maxUploadKb()]]);$file=$data['media'];$path=$file->store('galleries/media','public');$type=str_starts_with((string)$file->getMimeType(),'video/')?'video':'image';return response()->json(['path'=>$path,'url'=>Storage::disk('public')->url($path),'type'=>$type,'name'=>$file->getClientOriginalName()]);}
