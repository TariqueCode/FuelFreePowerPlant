...
class GalleryController extends Controller
{
    ...
    public function uploadMedia(Request $request): JsonResponse{$data=$request->validate(['media'=>['required','file','mimes:jpg,jpeg,png,webp,gif,mp4,webm,mov','max:102400']]);$file=$data['media'];$path=$file->store('galleries/media','public');$type=str_starts_with((string)$file->getMimeType(),'video/')?'video':'image';return response()->json(['path'=>$path,'url'=>Storage::disk('public')->url($path),'type'=>$type,'name'=>$file->getClientOriginalName()]);}
    ...
    {
        $data=$request->validate(['title'=>['required','string','max:255'],'excerpt'=>['nullable','string','max:500'],'cover_image'=>['nullable','image','mimes:jpg,jpeg,png,webp,gif','max:20480'],'status'=>['required','in:draft,published'],'sort_order'=>['nullable','integer','min:0'],'published_at'=>['nullable','date'],'media'=>['nullable','array'],'media.*.path'=>['required_with:media','string','max:500'],'media.*.type'=>['required_with:media','in:image,video'],'media.*.name'=>['nullable','string','max:255']]);
