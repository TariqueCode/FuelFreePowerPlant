...
class ManagementController extends Controller
{
    ...
    public function folderStore(Request $request): RedirectResponse
    {
        ...
        $folder = new ManagementProfileFolder(); $folder->name = $data['name']; $folder->slug = $this->uniqueFolderSlug($data['name']); $folder->status = $data['status'];
        $folder->sort_order = (int)(ManagementProfileFolder::query()->max('sort_order') ?? 0) + 1; $folder->save();
    ...
    {
        $data = $request->validate(['name' => ['required','string','max:120'], 'status' => ['required','in:draft,published']]);
        $folder->name = $data['name']; $folder->slug = $this->uniqueFolderSlug($data['name'], $folder->id); $folder->status = $data['status']; $folder->save();
    ...
    public function folderDestroy(ManagementProfileFolder $folder): RedirectResponse
    {
        ...
        $folder->delete(); return back()->with('status','Profile folder deleted.');
    }

    public function folderReorder(Request $request): JsonResponse
    {
        $data = $request->validate(['order'=>['required','array'],'order.*'=>['integer']]);
        $folders = ManagementProfileFolder::query()->whereIn('id',$data['order'])->get()->keyBy('id');
    ...
    {
        return view('admin.management.form', ['member'=>new SiteContentItem(['type'=>'management','status'=>'draft']), 'folders'=>ManagementProfileFolder::query()->orderBy('sort_order')->orderBy('id')->get()]);
    }
    ...
    public function edit(SiteContentItem $member): View
    {
        abort_unless($member->type === 'management',404);
        return view('admin.management.form', ['member'=>$member,'folders'=>ManagementProfileFolder::query()->orderBy('sort_order')->orderBy('id')->get()]);
    }

    public function update(Request $request, SiteContentItem $member): RedirectResponse
    {
        abort_unless($member->type === 'management',404); $this->save($member,$request); return redirect()->route('admin.profile-builder.index')->with('status','Profile updated.');
    }

    public function destroy(SiteContentItem $member): RedirectResponse
    {
        abort_unless($member->type === 'management',404);
        foreach([$member->image_path,$member->visiting_card_path] as $path) if($path) Storage::disk('public')->delete($path);
        $member->delete(); return back()->with('status','Profile deleted.');
    }

    public function toggle(SiteContentItem $member): RedirectResponse
    {
        abort_unless($member->type === 'management',404); abort_unless(request()->user()->hasPermission('website.publish'),403,'Publishing profiles requires publishing permission.');
        $member->status = $member->status === 'published' ? 'draft' : 'published'; if($member->status === 'published' && !$member->published_at) $member->published_at = now(); $member->save();
        return back()->with('status',$member->status === 'published' ? 'Profile activated.' : 'Profile deactivated.');
    }

    public function reorder(Request $request): JsonResponse
    {
        $data=$request->validate(['order'=>['required','array'],'order.*'=>['integer']]); $members=SiteContentItem::query()->where('type','management')->whereIn('id',$data['order'])->get()->keyBy('id');
        foreach($data['order'] as $position=>$id) if(isset($members[$id])) $members[$id]->update(['sort_order'=>$position+1]); return response()->json(['ok'=>true]);
    }
    ...
    private function save(SiteContentItem $member, Request $request): void
    {
        ...
        $data=$request->validate([
            'management_profile_folder_id'=>['required','integer','exists:management_profile_folders,id'],'title'=>['required','string','max:255'],'designation'=>['required','string','max:255'],'phone'=>['required','string','max:50'],'email'=>['nullable','email','max:255'],'content'=>['nullable','string'],'status'=>['required','in:draft,published'],'published_at'=>['nullable','date'],
            'profile_photo'=>['nullable','image','mimes:jpg,jpeg,png,webp','max:5120'],'visiting_card'=>['nullable','file','mimes:jpg,jpeg,png,webp,pdf','max:10240'],'remove_profile_photo'=>['nullable','boolean'],'remove_visiting_card'=>['nullable','boolean'],
        ]);
    ...
    {
        $base=Str::slug($name)?:'profile-folder'; $slug=$base; $counter=2;
        while(ManagementProfileFolder::query()->where('slug',$slug)->when($ignoreId !== null,fn($q)=>$q->where('id','!=',$ignoreId))->exists()) $slug=$base.'-'.($counter++);
        return $slug;
    }
}
