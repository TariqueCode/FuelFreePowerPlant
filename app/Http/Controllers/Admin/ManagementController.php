...
class ManagementController extends Controller
{
    ...
    private function save(SiteContentItem $member,Request $request): void
    {
        ...
        if($request->hasFile('profile_photo')){$this->deletePublicUpload($member->image_path);$member->image_path=$this->storePublicUpload($request->file('profile_photo'),'management/photos');}
        if($request->hasFile('visiting_card')){$this->deletePublicUpload($member->visiting_card_path);$member->visiting_card_path=$this->storePublicUpload($request->file('visiting_card'),'management/visiting-cards');}
        $member->save();
    }

    private function storePublicUpload($file,string $directory): string
    {
        $extension=strtolower((string)$file->getClientOriginalExtension());
        $allowed=['jpg','jpeg','png','webp','pdf'];
        if(!in_array($extension,$allowed,true))$extension='bin';
    ...
    private function deletePublicUpload(?string $path): void
    {
        ...
        if($root===false||$target===false||!str_starts_with($target,$root.DIRECTORY_SEPARATOR))return;
