...
class ManagementController extends Controller
{
    ...
    {
        $data=$request->validate(['title'=>['required','string','max:255'],'designation'=>['required','string','max:255'],'phone'=>['required','string','max:50'],'email'=>['nullable','email','max:255'],'content'=>['nullable','string'],'status'=>['required','in:draft,published'],'sort_order'=>['nullable','integer','min:0'],'published_at'=>['nullable','date'],'profile_photo'=>['nullable','image','mimes:jpg,jpeg,png,webp','max:5120'],'visiting_card'=>['nullable','file','mimes:jpg,jpeg,png,webp,pdf','max:10240'],'remove_profile_photo'=>['nullable','boolean'],'remove_visiting_card'=>['nullable','boolean']]);
        $member->type='management';$member->title=$data['title'];$member->slug=Str::slug($data['title']);$member->designation=$data['designation'];$member->excerpt=$data['designation'];$member->phone=$data['phone'];$member->email=$data['email']??null;$member->content=$data['content']??null;$member->status=$data['status'];$member->sort_order=$data['sort_order']??($member->sort_order?:1);$member->published_at=$data['published_at']??null;
