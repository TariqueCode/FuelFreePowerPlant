...
class SiteSliderController extends Controller
{
    public function index(): View
    {
        ...
        $stats = [
            'total' => $sliders->count(),
            'live' => $sliders->filter(fn (SiteSlider $slider) =>
                $slider->is_published
                && (! $slider->starts_at || $slider->starts_at->lte($now))
                && (! $slider->ends_at || $slider->ends_at->gte($now))
            )->count(),
            'scheduled' => $sliders->filter(fn (SiteSlider $slider) =>
                $slider->is_published
                && $slider->starts_at
                && $slider->starts_at->gt($now)
            )->count(),
            'expired' => $sliders->filter(fn (SiteSlider $slider) =>
                $slider->is_published
                && $slider->ends_at
                && $slider->ends_at->lt($now)
            )->count(),
            'draft' => $sliders->where('is_published', false)->count(),
        ];
    ...
    public function update(Request $request, SiteSlider $slider): RedirectResponse
    {
        if ($request->boolean('toggle')) {
            abort_unless(
                $request->user()->hasPermission('website.publish'),
                403,
                'Publishing sliders requires publishing permission.'
            );
    ...
    public function reorder(Request $request): JsonResponse
    {
        ...
        if (
            ...
            return response()->json([
                'ok' => false,
                'message' => 'The slider list changed. Refresh the page and try again.',
            ], 422);
        }
    ...
    private function save(SiteSlider $slider, Request $request): void
    {
        ...
        abort_unless(
            ! $publishing || $request->user()->hasPermission('website.publish'),
            403,
            'Publishing sliders requires publishing permission.'
        );
        ...
        $data = $request->validate([
            ...
            'image.mimes' => 'Slider image must be a JPG, PNG or WebP image.',
