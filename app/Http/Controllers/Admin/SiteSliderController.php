...
class SiteSliderController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        try { $this->save($slider, $request); }
        catch (Throwable $e) {
            if ($e instanceof HttpExceptionInterface) throw $e;
            Log::error('Slider creation failed.', ['user_id'=>$request->user()?->id,'exception'=>$e]);
    ...
    public function edit(SiteSlider $slider): View { return view('admin.sliders.form', compact('slider')); }
    ...
    public function update(Request $request, SiteSlider $slider): RedirectResponse
    {
        ...
        try { $this->save($slider, $request); }
            ...
            if ($e instanceof HttpExceptionInterface) throw $e;
            Log::error('Slider update failed.', ['user_id'=>$request->user()?->id,'slider_id'=>$slider->id,'exception'=>$e]);
            return back()->withInput()->with('error', 'The slider could not be saved. Please check the image and try again.');
        }
    ...
    public function reorder(Request $request): JsonResponse
    {
        ...
        foreach ($data['order'] as $position=>$id) if (isset($sliders[$id])) $sliders[$id]->update(['sort_order'=>$position+1]);
    ...
    {
        $publishing = $request->boolean('is_published') || strtolower((string)$request->input('status')) === 'published';
        ...
        $data = $request->validate([
            ...
            'image'=>[$slider->exists ? 'nullable' : 'required','file'],
        ...
        try {
            if ($request->hasFile('image')) {
                ...
                if (! $file->isValid()) throw new \RuntimeException('The image upload failed. Please choose the image again.');
                $imageInfo = @getimagesize($file->getRealPath());
                $extension = match ($imageInfo[2] ?? null) { IMAGETYPE_JPEG=>'jpg', IMAGETYPE_PNG=>'png', IMAGETYPE_WEBP=>'webp', default=>null };
                if (! $imageInfo || ! $extension) throw new \RuntimeException('Slider image must be a valid JPG, PNG or WebP image.');
                $newPath = 'site-sliders/'.Str::uuid().'.'.$extension;
                $stored = Storage::disk('public')->putFileAs('site-sliders',$file,basename($newPath));
                if (! $stored) throw new \RuntimeException('The server could not save the image.');
                $data['image_path']=$stored;
            }
            unset($data['image']);
            $data['is_published']=$publishing;
            if (! $slider->exists) $data['sort_order']=((int)SiteSlider::query()->max('sort_order'))+1;
