...
class HomeController
{
    public function __invoke(): Response
    {
        ...
        // A published profile folder is the canonical source for the homepage
        // Board of Directors section. If an older homepage configuration has no
        // folder/IDs yet, fall back to the first published folder instead of
        // rendering an empty section after the section is enabled.
        if (isset($enabledSections['management']) && $managementFolderId <= 0) {
            $managementFolderId = (int) (ManagementProfileFolder::query()
        ...
        if ($managementFolderId > 0) {
            if ($managementSelectedIds) {
