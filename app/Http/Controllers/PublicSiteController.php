...
class PublicSiteController
{
    public function show(string $section): View
    {
        ...
        if($section==='about-us'){
            ...
            $items=SiteContentItem::published()->where('type','gallery')->orderBy('sort_order')->latest('published_at')->get()->map(function(SiteContentItem $item){
                $item->safe_content=$this->safeGalleryContent($item->content);
                return $item;
            });
            return view('gallery.index',compact('items','brand'));
        }
    ...
    private function safeGalleryContent(?string $content): string
    {
        ...
        return strip_tags($content,'<a><p><br><div><span><figure><figcaption><img><picture><source><video><iframe>');
    }
}
