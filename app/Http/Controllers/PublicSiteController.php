...
class PublicSiteController
{
    ...
    public function show(string $section): View
    {
        ...
        if($section==='about-us'){$aboutItem=SiteContentItem::published()->where('type','company')->where('slug','about-us')->first();if($aboutItem){return view('site.company-page',['item'=>$aboutItem,'brand'=>$brand,'backRoute'=>route('home'),'backLabel'=>'Back to Home']);}}
        ...
        if($section==='about-us'){$companyItems=SiteContentItem::published()->where('type','company')->where('slug','about-us')->orderBy('sort_order')->orderBy('title')->get();}elseif($section==='plants'){$items=SiteContentItem::published()->where('type','plants')->orderBy('sort_order')->orderBy('title')->get();}elseif($section==='future-project'){$items=SiteContentItem::published()->where('type','future-project')->orderBy('sort_order')->orderBy('title')->get();}elseif($section==='career'){$items=SiteContentItem::published()->whereIn('type',['career','careers','job'])->orderBy('sort_order')->orderBy('title')->get();}elseif($section==='solutions'){$items=SiteContentItem::published()->where('type','solution')->orderBy('sort_order')->orderBy('title')->get();}
        $titles=['about-us'=>'About Us','plants'=>'Plants','future-project'=>'Future Project','career'=>'Career','solutions'=>'Solutions'];return view('site.section',compact('section','titles','page','pages','brand','companyItems','items','projects'));
    }
