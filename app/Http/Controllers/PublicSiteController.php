...
class PublicSiteController
{
    public function show(string $section): View
    {
        ...
        $page=$section==='about-us'?null:CmsPage::query()->where('slug',$section)->where('is_published',true)->first();
        $companyItems=collect();$items=collect();$projects=collect();
        if($section==='about-us'){$companyItems=SiteContentItem::published()->where('type','company')->orderBy('sort_order')->orderBy('title')->get();}
        elseif($section==='plants'){$projects=PowerPlant::query()->orderByRaw("CASE WHEN status='operational' THEN 0 ELSE 1 END")->orderBy('name')->get();}
    ...
    public function showGallery(SiteContentItem $item): View
    {
        abort_unless($item->status==='published' && $item->type==='gallery',404);
        $settings=SystemSetting::query()->whereIn('key',['company.name','company.logo_path','company.tagline'])->pluck('value','key');
        $brand=['name'=>$settings->get('company.name')?:config('fuelfree.company.name'),'logo_path'=>$settings->get('company.logo_path'),'tagline'=>$settings->get('company.tagline')?:config('fuelfree.company.tagline')];
        $item->safe_content=$this->safeGalleryContent($item->content);
        return view('gallery.show',compact('item','brand'));
    }

    private function safeGalleryContent(?string $content): string
    {
        if(!$content){return '';}
        return strip_tags($content,'<a><p><br><div><span><figure><figcaption><img><picture><source><video><iframe><ul><ol><li>');
    }
}
