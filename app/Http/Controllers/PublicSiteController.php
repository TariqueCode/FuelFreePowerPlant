...
class PublicSiteController
{
    public function show(string $section): View
    {
        ...
        if($section==='gallery'){
            $galleries=SiteContentItem::query()->where('type','gallery')->where('status','published')->withCount('galleryMedia')->orderBy('sort_order')->latest('created_at')->get();
            return view('gallery.index',compact('galleries','brand'));
        }
