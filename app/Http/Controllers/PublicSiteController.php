...
class PublicSiteController
{
    public function show(string $section): View
    {
        ...
        if($section==='about-us'){$companyItems=SiteContentItem::published()->where('type','company')->orderBy('sort_order')->orderBy('title')->get();}elseif($section==='plants'){$projects=PowerPlant::query()->orderByRaw("CASE WHEN status='operational' THEN 0 ELSE 1 END")->orderBy('name')->get();}elseif($section==='future-project'){$projects=PowerPlant::query()->whereRaw("LOWER(status) != 'operational'")->orderBy('name')->get();$items=SiteContentItem::published()->whereIn('type',['future-project','project'])->orderBy('sort_order')->orderBy('title')->get();}elseif($section==='career'){$items=SiteContentItem::published()->whereIn('type',['career','careers','job'])->orderBy('sort_order')->orderBy('title')->get();}elseif($section==='solutions'){$items=SiteContentItem::published()->where('type','solution')->orderBy('sort_order')->orderBy('title')->get();}
